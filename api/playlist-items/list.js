...
export default async function handler(req, res) {
  try {
    ...
    // WICHTIG: View statt Tabelle + neue Felder im select
    const path =
      `/rest/v1/playlist_items_with_age` +
      `?select=position,track_id,track_name,artist_names,album_name,duration_ms,duration_formatted,popularity,preview_url,cover_url,added_at,age_days,age_label` +
