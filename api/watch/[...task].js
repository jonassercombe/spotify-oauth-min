...
function backoffMs(changed){
  // simple heuristic: aktiv -> 5 min, sonst 15 min (kannst du später exponentiell machen)
            ...
            if (r.status === 429){
              ...
              if (r.ok){
                ...
                if (current && current !== row.snapshot_id) {
                  ...
                  // optional: NICHT sofort snapshot_id setzen → setz es erst nach erfolgreichem Track-Sync
                  marked++;
                }
...
async function runSyncNeeded({ connection_id, limit="50", concurrency="3" }){
  // Playlists mit needs_sync holen
  ...
  // Wir rufen DEINE bestehende sync-items Route auf
...
async function runCronCheckAll({ limit, concurrency }){
  ...
  for (const c of conns) {
    const res = await runCheckUpdates({ connection_id: c.id, limit, concurrency });
    res?.ok ? ok++ : fail++;
    await sleep(80);
  }
...
async function runCronSyncAll({ limit, concurrency }){
  ...
  let ok=0, fail=0;
  for (const c of conns) {
    const res = await runSyncNeeded({ connection_id: c.id, limit, concurrency });
    res?.ok ? ok++ : fail++;
    await sleep(80);
  }
  return { ok:true, connections: conns.length, dispatched_ok: ok, dispatched_fail: fail };
...
export default async function handler(req, res){
  try{
    ...
    const path = url.pathname; // /api/watch/...
    const qs = Object.fromEntries(url.searchParams.entries());

    // Routing nach Pfadende
    if (path.endsWith("/check-updates")) {
      if (req.method !== "POST") return res.status(405).json({ error:"method_not_allowed" });
      if (!checkAppSecret(req)) return res.status(401).json({ error:"unauthorized" });
      const body = await (async()=>{ if (req.body && typeof req.body==="object") return req.body;
    ...
    if (path.endsWith("/cron-check-all")) {
    ...
    if (path.endsWith("/cron-sync-all")) {
      if (!checkCronAuth(req)) return res.status(401).json({ error:"unauthorized_cron" });
