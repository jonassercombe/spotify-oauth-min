...
export default async function handler(req, res) {
  ...
  try {
    ...
    const limit = encodeURIComponent(req.query.limit || "200");        // Playlists pro Connection pro Run
    const concurrency = encodeURIComponent(req.query.concurrency || "4");
    ...
    for (const c of conns) {
      const url = `${base}/api/watch/check-updates?limit=${limit}&concurrency=${concurrency}`;
      ...
      // kleines Pacing, schont Rate Limits und Serverless Cold Starts
      await sleep(80);
    }
