...
export function buildCreativeDeck(seed, count = 8) {
  ...
  const toneMix = shuffle(
    Array.from({ length: count }, (_, index) => BATCH_TONE_MIX[index % BATCH_TONE_MIX.length]),
    random
  );
  const usedHookStructures = new Set();
  const footageFirstCount = Math.min(count, random() < 0.5 ? 2 : 3);
  const footageFirstSlots = new Set();
  while (footageFirstSlots.size < footageFirstCount) {
    footageFirstSlots.add(1 + Math.floor(random() * count));
  }
  const recipes = [];
  for (let index = 0; index < count; index += 1) {
    ...
    recipes.push({
      ...
      risk_level: risk,
      workflow: footageFirstSlots.has(index + 1) ? "footage_first" : "concept_first",
      serendipity_words: [
