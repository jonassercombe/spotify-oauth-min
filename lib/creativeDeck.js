...
const BATCH_TONE_PROFILES = [
  ...
  ["accessible", "accessible", "accessible", "accessible", "creative", "creative", "wildcard", "wildcard"],
  ["accessible", "accessible", "accessible", "creative", "creative", "creative", "creative", "wildcard"],
];

const HOOK_FOCUS_PROFILES = [
  ["audience story", "audience story", "playlist benefit", "playlist benefit", "identity", "listening moment", "scene-led", "wildcard"],
  ["audience story", "emotional truth", "playlist benefit", "playlist benefit", "listening moment", "identity", "scene-led", "wildcard"],
  ["audience story", "identity", "playlist benefit", "playlist benefit", "invitation", "listening moment", "scene-led", "wildcard"],
];
...
export function buildCreativeDeck(seed, count = 8) {
  ...
  const hookFocusMix = shuffle(draw(HOOK_FOCUS_PROFILES, random), random);
  const toneMix = shuffle(
    Array.from({ length: count }, (_, index) => toneProfile[index % toneProfile.length]),
    random
  );
  const usedHookStructures = new Set();
  const lateralSearchCount = Math.min(count, 1 + Math.floor(random() * 3));
  const lateralSearchSlots = new Set();
  while (lateralSearchSlots.size < lateralSearchCount) {
    lateralSearchSlots.add(1 + Math.floor(random() * count));
  }
  const recipes = [];
  for (let index = 0; index < count; index += 1) {
    ...
    recipes.push({
      ...
      hook_focus: hookFocusMix[index % hookFocusMix.length],
