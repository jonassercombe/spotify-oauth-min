// api/oauth/spotify/callback.js
// Node 18+ (Vercel) hat global fetch.
...
function enc(plain) {
  // ENC_SECRET: 32-Byte Hex (64 Zeichen). z.B. via `openssl rand -hex 32`
...
// NEW: insert-if-not-exists (ignores duplicates on primary key)
async function sbInsertIgnore(path, body, onConflictCol) {
  const url = `${assertEnv("SUPABASE_URL")}${path}?on_conflict=${encodeURIComponent(onConflictCol)}`;
  const r = await fetch(url, {
    method: "POST",
    headers: {
      apikey: assertEnv("SUPABASE_SERVICE_ROLE_KEY"),
  ...
  if (!r.ok && r.status !== 409) {
    throw new Error(`Supabase INSERT (ignore) ${path} failed: ${r.status} ${await r.text()}`);
  }
}
...
export default async function handler(req, res) {
  try {
    ...
    // DEBUG: show exact error from Spotify if it fails
    if (!meResp.ok) {
    ...
    // Ensure app_user exists (ignore if duplicate)
    await sbInsertIgnore("/rest/v1/app_users", { bubble_user_id }, "bubble_user_id");
    ...
    // Decide refresh_token_enc (fallback to existing if Spotify didn't send a new one)
    ...
    // Upsert: PATCH if exists, else POST
    if (existingRow) {
      await sbPatch(
        `/rest/v1/spotify_connections?` +
          `bubble_user_id=eq.${encodeURIComponent(bubble_user_id)}&` +
